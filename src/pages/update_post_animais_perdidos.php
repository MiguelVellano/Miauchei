...
if(isset($_POST['update_post_btn'])){
    ...
    if($new_image != ""){
        ...
        $stmt = $conn->prepare("UPDATE posts_animais_perdidos SET image = ? , nome_pet = ?, raca = ?, sexo = ?, data_que_perdeu = ?, idade_pet = ?, descricao = ? WHERE id = ?");
        $stmt->bind_param("sssssssi",$image_name,$nome_pet,$raca,$sexo,$data_perdeu,$idade,$descricao,$post_id);
